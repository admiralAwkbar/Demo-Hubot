...
# Authors:
#   Taytay
#   Using code written by: spajus, patcon, and parkr , LukasG
...
module.exports = (robot) ->
  ...
  ####################################
  # BASIC LISTENER FOR CI CD PROCESS #
  ####################################
...
################################################################################
  ############################################
  # BASIC LISTENER FOR PREFLIGHT PUSH EVENTS #
  ############################################
  robot.router.post "/hubot/github-preflight-event-push", (req, res) ->
    try
      if (debug)
        robot.logger.info("Github post received: ", req)
      eventBody =
      ...
      robot.emit "github-preflight-event-push", eventBody
    catch error
      robot.logger.error "Github repo webhook listener error: #{error.stack}. Request: #{req.body}"
...
################################################################################
  ############################################
  # BASIC LISTENER FOR PREFLIGHT PULL EVENTS #
  ############################################
  robot.router.post "/hubot/github-preflight-event-pull", (req, res) ->
    try
      if (debug)
        robot.logger.info("Github post received: ", req)
      eventBody =
      ...
      robot.emit "github-preflight-event-pull", eventBody
    catch error
      robot.logger.error "Github repo webhook listener error: #{error.stack}. Request: #{req.body}"
