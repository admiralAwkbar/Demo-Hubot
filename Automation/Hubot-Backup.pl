...
# GLOBALS: #
############
my $botName = undef;            # Name of the bot being installed
...
sub GetHubotName
{
  # Need to find the hubot name
  my $command="sudo find /opt -type f -name hubot.env | awk -F '/' '{print \$3}' 2>&1";
  my $result=`$command`;

  if ($?!=0)
  {
    print "ERROR! Failed to gain hubot name!\n";
    exit(1);
  }
  else
  {
...
{
  print "-------------------------------------------------------------------\n";
  print "This automation will backup the Hubot\n";
